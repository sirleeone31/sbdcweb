...
import com.sbdc.sbdcweb.info.repository.InfoOpenTwoRepository;
 ...
 */
@Service
public class InfoOpenTwoServiceImpl extends InfoServiceImpl<InfoOpenTwo, Long> implements InfoOpenTwoService {
	private static final Logger logger = LoggerFactory.getLogger(InfoOpenTwoServiceImpl.class);

	private final InfoOpenTwoRepository infoOpenTwoRepository;
    private final FileManager fileManager;
    private final String infoCode = "infoopen2";
	...
	@Autowired
	public InfoOpenTwoServiceImpl(InfoOpenTwoRepository infoOpenTwoRepository, FileManager fileManager) {
        super(infoOpenTwoRepository);
        super.setInfoCode(infoCode);
        super.setPathName(pathName);
        super.setBbsCode("");
        this.infoOpenTwoRepository = infoOpenTwoRepository;
    ...
    public List<InfoOpenTwo> selectInfoList(String type) {
		List<InfoOpenTwo> infoList = infoOpenTwoRepository.findByTypeOrderByInfoKeyDesc(type);
	 ...
	 */
    @Override
    public InfoOpenTwo insertInfo(InfoOpenTwo infoRequest, MultipartFile upload) {
    	if (upload != null) {
            // 업로드 파일명
     ...
     */
    @Override
    public InfoOpenTwo updateInfo(Long id, InfoOpenTwo infoRequest, MultipartFile upload) {
    	InfoOpenTwo info = super.select(id);
    ...
    public Map<String, Object> delete(Long id) {
    	...
    	if (info != null) {
	        try {
	        	...
	        	infoOpenTwoRepository.delete(info);
	    		deleteYN = true;
	    	} catch (Exception e) {
