...
import com.sbdc.sbdcweb.info.repository.InfoMoneyTwoRepository;

/**
 * 청탁금지법 상담 및 조치 ServiceImpl
 * 
 * @author  : 이승희
 * @version : 1.0
 * @since   : 1.0
 ...
 */
@Service
public class InfoMoneyTwoServiceImpl extends InfoServiceImpl<InfoMoneyTwo, Long> implements InfoMoneyTwoService {
	private static final Logger logger = LoggerFactory.getLogger(InfoMoneyTwoServiceImpl.class);

	private final InfoMoneyTwoRepository infoMoneyTwoRepository;
    private final FileManager fileManager;
    private final String infoCode = "money2";
	...
	@Autowired
	public InfoMoneyTwoServiceImpl(InfoMoneyTwoRepository infoMoneyTwoRepository, FileManager fileManager) {
        super(infoMoneyTwoRepository);
        super.setInfoCode(infoCode);
        super.setPathName(pathName);
        super.setBbsCode("");
        this.infoMoneyTwoRepository = infoMoneyTwoRepository;
     ...
     */
    @Override
    public List<InfoMoneyTwo> selectInfoList() {
        List<InfoMoneyTwo> infoList = super.selectInfoList();
	 ...
	 */
    @Override
    public InfoMoneyTwo insertInfo(InfoMoneyTwo infoRequest, MultipartFile upload) {
    	if (upload != null) {
            // 업로드 파일명
     ...
     */
    @Override
    public InfoMoneyTwo updateInfo(Long id, InfoMoneyTwo infoRequest, MultipartFile upload) {
    	InfoMoneyTwo info = super.select(id);
    ...
    public Map<String, Object> delete(Long id) {
    	...
    	if (info != null) {
	        try {
	        	...
	        	infoMoneyTwoRepository.delete(info);
	    		deleteYN = true;
	    	} catch (Exception e) {
