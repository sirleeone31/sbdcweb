 ...
 */
@Service
public class BoardBidServiceImpl extends BoardServiceImpl implements BoardBidService {
	private static final Logger logger = LoggerFactory.getLogger(BoardBidServiceImpl.class);

    private final BoardAttachService boardAttachService;
    private final CommonUtils commonUtils;
    private final String boardCode = "bbs";
    private final String bbsCode = "bid";
	...
	@Autowired
	public BoardBidServiceImpl(BoardRepository boardRepository, BoardAttachService boardAttachService, CommonUtils commonUtils, FileManager fileManager, ImageManager imageManager) {
	...
	public List<BidAllDto> selectBoardBidList() {
		List<BidAllDto> boardBidList = new ArrayList<BidAllDto>();
		...
		for (Board board : boardList) {
		     ...
		     boardBidList.add(new BidAllDto(
		    		 board.getArticleKey(),
		    		 board.getArticleNo(),
		    		 board.getSubject(),
		    		 board.getRegDate(),
		    		 board.getEndDate(),
		    		 board.getContractType(),
		    		 board.getComplete()));
		}
	...
	public BidOneDto selectBoardBid(Long id, String role) {
		...
		if (board != null) {
			...
			boardBid.setFile(file);
			boardBid.setBoardAttach(boardAttachService.selectBoardAttachList(id));

			if (board.getComplete().equals("Y") && role.equals("user")) {
				boardBid.setContent1("마감된 입찰공고 입니다.");
			}
		...
		try {
			if (board != null) {
				...
				for (BoardAttach fileOne : fileList) {
					...
					boardBid.setFile(fileAll);
					boardBid.setBoardAttach(fileList);
    ...
    public Board updateBoard(Long id, Board boardRequest, BoardAttach attach, List<MultipartFile> upload) {
		...
		if (board != null) {
			...
			if (boardRequest.getContractType() != null && !boardRequest.getContractType().equals("")) {
				board.setContractType(boardRequest.getContractType());
			}
	...
	public List<BidAllDto> selectBoardBidTopList() {
		List<BidAllDto> boardBidList = new ArrayList<BidAllDto>();
//		List<Board> boardList = super.selectBoardList();
//		List<Board> boardList = boardRepository.selectTopByBbsCode("bid", PageRequest.of(0, 3));
...
//		List<BidOneDto> bidMain = boardRepository.selectTop3BidMain("bid");
//
//    	// SQL Server 버전 업 후 PageRequest 로 변경
//    	List<BidOneDto> bidTopMain = new ArrayList<>();
//
//    	bidTopMain.add(0, bidMain.get(0));
//    	bidTopMain.add(1, bidMain.get(1));
//    	bidTopMain.add(2, bidMain.get(2));
//
//    	return bidTopMain;
//        return boardBidRepository.selectTop3BidMain("bid", PageRequest.of(0, 3));
    }
